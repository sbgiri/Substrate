...
extension TypeRegistry: TypeRegistryProtocol {
    ...
    public func decode(keyFrom decoder: ScaleDecoder) throws -> AnyStorageKey {
        ...
        let info1 = try info(forKey: prefix)
        if let kt = _keys["\(info1.module).\(info1.field)"] {
            return try _decodingError {
                try kt.init(parsingPathFrom: decoder, registry: self)
            }
        } else {
            return try _decodingError  {
                try DStorageKey(module: info1.module, field: info1.field, decoder: decoder, registry: self)
            }
        }
    }
    ...
    public func decode(eventFrom decoder: ScaleDecoder) throws -> AnyEvent {
        ...
        let info1 = try info(forEvent: header)
        if let event = _events["\(info1.module).\(info1.event)"] {
            return try _eventDecodingError(module: info1.module, event: info1.event) {
                try event.init(decodingDataFrom: decoder, registry: self)
            }
        } else {
            return try _eventDecodingError(module: info1.module, event: info1.event) {
                try DEvent(module: info1.module, event: info1.event, decoder: decoder, registry: self)
            }
        }
    }
    ...
    public func decode(callFrom decoder: ScaleDecoder) throws -> AnyCall {
        ...
        let info1 = try info(forCall: header)
        if let call = _calls["\(info1.module).\(info1.call)"] {
            return try _callDecodingError(module: info1.module, funct: info1.call) {
                try call.init(decodingParamsFrom: decoder, registry: self)
            }
        } else {
            return try _callDecodingError(module: info1.module, funct: info1.call) {
                try DCall(module: info1.module, function: info1.call, decoder: decoder, registry: self)
            }
        }
    }
}
